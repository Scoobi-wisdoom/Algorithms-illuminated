...
A pivot stays at the beginning of the array during the partition subroutine.
```text
Initialization
                     ┌─────┬─────┬─────┬─────┬─────┬─────┬─────┐
partition target     │  p  │     │ ... │     │     │     │     │
...
Proof:   
Let $` z_i `$ and $` z_j `$ denote the ith and jth smallest elements of the input array. Define $` X_{ij}(\omega) `$ as the number of comparisons between $` z_i `$ and $` z_j `$ in QuickSort when the pivots are specifed by $` \omega `$ for every fixed choise of pivots $` \omega `$. The total number of comparisons made by QuickSort is expressed as:
```math
\begin{align}
